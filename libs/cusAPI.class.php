...
/**
 * Initialization Class for ContactUs.com API (suffix according to plugin _SBr)
 * @api
 * @version 0.4
...
class cUsComAPI_SBr {
     ...
     * @api
     */
    var $enviroment = cUsSBr_API_ENV;
    var $wpApiAccountKey = cUsSBr_API_ACC;
    var $wpApiKey = cUsSBr_API_AKY;
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        if( $this->_isCurl() ){
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: wpcbr|1.2.3|' . $this->getIP()));
    ...
    public function createCustomer($postData, $credential) {
        ...
        if (!strlen($postData['fname'])) {
            ...
            if( $this->_isCurl() ){
                ...
                $strCURLOPT .= '&Template_Desktop_Tab=invisibleTab';
                $strCURLOPT .= '&Auto_Activate=1';
                $strCURLOPT .= '&API_Credentials=1';
                $strCURLOPT .= '&Promotion_Code=WPCBr';
                $strCURLOPT .= '&Version=wpcbr|1.2.3';
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    ...
                    'X-ContactUs-Signature: wpcbr|1.2.3|' . $this->getIP()
                ));
    ...
    public function resetData() {
        ...
        //PLUGIN OPTIONS
        delete_option('cUsSBr_settings');
        delete_option('cUsSBr_settings_userData');
        delete_option('cUsSBr_settings_FORM');
        delete_option('cUsSBr_settings_HOME');
        delete_option('cUsSBr_settings_step1');
        delete_option('cUsSBr_settings_form_key');
        delete_option('cUsSBr_settings_inlinepages');
        delete_option('cUsSBr_settings_tabpages');
        delete_option('cUsSBr_settings_userCredentials');
        delete_option('cUsSBr_settings_default_deep_link_view');
        delete_option('cUsSBr_settings_form_id');

        cUsSBr_page_settings_cleaner(); //REMOVE CUSTOM SETTINGS BY PAGE
        cUsSBr_inline_shortcode_cleaner(); //REMOVE SHORTCODES
